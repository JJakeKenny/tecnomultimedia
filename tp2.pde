import ddf.minim.*; //carga la blioteca para poder trabajar con audio

//declaracion de variables
 Minim minim;
 AudioPlayer player; //creacion de objeto para poder reproducir audio
 PImage fondo5;
...
//comienzo de creditos
  if (inicio) {
    ...
    if (opacidadTI <= -10) {
      moverYT -= 3;
      timer ++;
      player.play(); //reproduccion del sonido 
    }
  } else {
    opacidadTI += 2;
    if (opacidadTI >=255) {
